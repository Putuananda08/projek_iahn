...
function get_nip_option($selected = ''){
    global $db;
    $rows = $db->get_results("SELECT nip,nama FROM karyawan ORDER BY nip");
    foreach($rows as $row){
        if($row->nip==$selected)
            $a.="<option value='$row->nip' selected>$row->nip - $row->nama</option>";
        else
            $a.="<option value='$row->nip'>$row->nip - $row->nama</option>";
    }
    return $a;
}
function get_nip_atasan_option($selected = ''){
    global $db;
    $rows = $db->get_results("SELECT nip_atasan, nama_atasan FROM atasan ORDER BY nip_atasan");
    foreach($rows as $row){
        if($row->nip_atasan==$selected)
            $a.="<option value='$row->nip_atasan' selected>[$row->nip_atasan]</option>";
        else
            $a.="<option value='$row->nip_atasan'>[$row->nip_atasan]</option>";
    }
    return $a;
}
...
function get_jenis_cuti_option($selected = ''){
    ...
    foreach($rows as $row){
        if($row->nama_cuti==$selected)
            $a.="<option value='$row->nama_cuti' selected>$row->id_cuti.$row->nama_cuti</option>";
        else
            $a.="<option value='$row->nama_cuti'>$row->id_cuti.$row->nama_cuti</option>";
    }
    return $a;
}
?>
